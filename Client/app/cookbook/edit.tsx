...
export default function EditCookbookScreen() {
  ...
  const { cookbooks, recipes, fetchRecipes, savedRecipes, fetchSavedRecipes, fetchCookBooks } = useRecipeStore();
  ...
  const [selectedRecipeIds, setSelectedRecipeIds] = useState<number[]>(
    (cookbook?.recipes || []).map((r) => r.recipeId)
  );
  ...
  useEffect(() => {
    if (cookbook) {
      ...
      setSelectedRecipeIds((cookbook.recipes || []).map((r) => r.recipeId));
    }
  }, [cookbook]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await recipeService.updateCookBook(id, payload);
      await fetchCookBooks();
      show('Cookbook updated successfully!', 'success');
      router.back();
    } catch (err: any) {
      const msg = err?.data
        ? Object.values(err.data).flat().join(', ')
        : (err?.message || 'Failed to update cookbook');
      show(msg, 'error');
    } finally {
      setLoading(false);
    }
  ...
  const handleDeleteRecipe = (recipeId: number) => {
    setSelectedRecipeIds(selectedRecipeIds.filter((rId) => rId !== recipeId));
  };

  const handleAddRecipeToList = (recipeId: number) => {
    ...
    return (
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.center}>
          <Text style={styles.emptySelectorText}>Cookbook not found.</Text>
        ...
        {showAddList && (
          <View style={styles.selectorDropdown}>
            ...
            {unselectedRecipes.length === 0 ? (
              <Text style={styles.emptySelectorText}>No more eligible recipes available.</Text>
            ) : (
                  ...
                  <View style={styles.selectorItemLeft}>
                    ...
                    <View style={{ marginLeft: 10 }}>
                      ...
                      <Text style={styles.selectorSubText}>
                        {recipe.authorId === currentUserId ? 'Your Recipe' : 'Saved Public Recipe'}
                      </Text>
                    </View>
                  </View>
